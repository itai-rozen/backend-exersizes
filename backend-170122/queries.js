...
'db.restaurants.find({"reviews.date":"16.1.61"})'
...
'db.restaurants.aggregate([{ $match: { name: "Hatzilim 2000" } }, { $unwind: "$reviews" }, { $group: { _id: "$name", avgScore: { $avg: "$reviews.score" } } }])'
